...
## Fase 6: Geração dos arquivos PDF e DOCX

### 6.1 Parâmetros de exportação

Gerar os arquivos automaticamente após o checklist, sem aguardar novo comando.

- **Diretório de saída:** `C:\Users\aldem\`
- **Nomes base:**
  - Capítulo 8: `capitulo-08-propriedade-divulgacao_[AAAAMMDD]`
  - Capítulo 9: `capitulo-09-responsabilidades_[AAAAMMDD]`
  - Capítulo 10: `capitulo-10-grupos-vulneraveis_[AAAAMMDD]`
...
### 6.3 Salvar os arquivos Markdown intermediários

Usar a ferramenta **Write** para salvar cada capítulo em arquivo Markdown separado no diretório de saída. Os arquivos Markdown são os arquivos-fonte de todas as conversões e devem ser mantidos pelo usuário para edições futuras.

**Capítulo 8:**
Caminho: `C:\Users\aldem\capitulo-08-propriedade-divulgacao_[AAAAMMDD].md`
```yaml
---
title: "Capítulo 8. Propriedades da Informação e Divulgação da Pesquisa"
lang: pt-BR
---
```

**Capítulo 9:**
Caminho: `C:\Users\aldem\capitulo-09-responsabilidades_[AAAAMMDD].md`
```yaml
---
title: "Capítulo 9. Responsabilidades do Pesquisador, da Instituição, do Promotor e do Patrocinador"
lang: pt-BR
---
```

**Capítulo 10:**
Caminho: `C:\Users\aldem\capitulo-10-grupos-vulneraveis_[AAAAMMDD].md`
```yaml
---
title: "Capítulo 10. Utilização de Grupos Vulneráveis"
lang: pt-BR
---
```

A lista de referências unificada deve ser incluída ao final do arquivo do Capítulo 10 ou como arquivo separado `referencias-cap8-9-10.md`, conforme preferência do usuário.

---

### 6.4 Gerar os arquivos DOCX

```bash
# Capítulo 8
pandoc "C:\Users\aldem\capitulo-08-propriedade-divulgacao_[AAAAMMDD].md" \
  -o "C:\Users\aldem\capitulo-08-propriedade-divulgacao_[AAAAMMDD].docx" \
  --metadata title="Capítulo 8. Propriedades da Informação e Divulgação da Pesquisa" \
  -V lang=pt-BR

# Capítulo 9
pandoc "C:\Users\aldem\capitulo-09-responsabilidades_[AAAAMMDD].md" \
  -o "C:\Users\aldem\capitulo-09-responsabilidades_[AAAAMMDD].docx" \
  --metadata title="Capítulo 9. Responsabilidades" \
  -V lang=pt-BR

# Capítulo 10
pandoc "C:\Users\aldem\capitulo-10-grupos-vulneraveis_[AAAAMMDD].md" \
  -o "C:\Users\aldem\capitulo-10-grupos-vulneraveis_[AAAAMMDD].docx" \
  --metadata title="Capítulo 10. Utilização de Grupos Vulneráveis" \
  -V lang=pt-BR
```

Os arquivos `.docx` gerados são compatíveis com **Microsoft Word** e podem ser importados diretamente no **Google Drive** para abertura e edição como Google Docs.
...
### 6.5 Gerar os arquivos PDF

Cada PDF deve conter, em todas as páginas: **cabeçalho centralizado** com o título do capítulo correspondente e **numeração de página** no rodapé direito.

Tentar os mecanismos de conversão na ordem abaixo, usando o primeiro que estiver disponível no sistema. Aplicar o mesmo mecanismo para os três capítulos.

**Opção A: wkhtmltopdf** (recomendado; instalar em https://wkhtmltopdf.org/downloads.html se necessário):

Repetir os dois passos abaixo para cada capítulo, substituindo `[nome-base]` e `[Título do Capítulo]`:
...
pandoc "[diretório]/[nome-base].md" \
  ...
  --metadata title="[Título do Capítulo]" \
  -V lang=pt-BR

# Passo 2: converter HTML para PDF com cabeçalho e numeração
wkhtmltopdf \
  --header-center "[Título do Capítulo]" \
...
Títulos a usar como `[Título do Capítulo]` em cada conversão:
- Capítulo 8: `Capítulo 8. Propriedades da Informação e Divulgação da Pesquisa`
- Capítulo 9: `Capítulo 9. Responsabilidades`
- Capítulo 10: `Capítulo 10. Utilização de Grupos Vulneráveis`
...
Escrever um arquivo `.tex` de cabeçalho para cada capítulo com a ferramenta Write antes da conversão. Exemplo para o Capítulo 9 (adaptar os demais):

```latex
\usepackage{fancyhdr}
\pagestyle{fancy}
\fancyhf{}
\fancyhead[C]{\small Capítulo 9. Responsabilidades}
\fancyfoot[R]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}
```

Em seguida executar (adaptar para cada capítulo):

```bash
pandoc "[diretório]/[nome-base].md" \
  -o "[diretório]/[nome-base].pdf" \
  --pdf-engine=xelatex \
  --include-in-header="[diretório]/cap[N]-header.tex" \
  -V lang=pt-BR \
  -V fontsize=12pt \
...
**Opção C: Chrome headless** (se nenhuma das opções anteriores estiver disponível; suporte a cabeçalho/rodapé via CSS @page é parcial):

```bash
pandoc "[diretório]/[nome-base].md" \
  -o "[diretório]/[nome-base].html" \
  --standalone --embed-resources \
  --css="[diretório]/academico-cap[N].css" \
  --metadata title="[Título do Capítulo]" \
  -V lang=pt-BR
...
O arquivo `academico-cap[N].css` deve incluir:

```css
@page {
  margin: 30mm 20mm 25mm 30mm;
  @top-center {
    content: "[Título do Capítulo]";
...
**Se nenhum mecanismo de PDF estiver disponível:** informar ao usuário que os DOCX foram gerados com sucesso e orientá-lo a usar **"Arquivo / Baixar como / PDF"** (Google Docs) ou **"Arquivo / Salvar como / PDF"** (Word) para obter os PDFs manualmente. Orientar a inserir manualmente o cabeçalho do capítulo e a numeração de páginas pelo menu de configuração de página do editor.
...
Arquivos gerados com sucesso:
  ...
  Capítulo 8:
    DOCX  ->  [caminho completo]/[nome-base-cap8].docx
    PDF   ->  [caminho completo]/[nome-base-cap8].pdf  (com cabeçalho e numeração de páginas)
    MD    ->  [caminho completo]/[nome-base-cap8].md

  Capítulo 9:
    DOCX  ->  [caminho completo]/[nome-base-cap9].docx
    PDF   ->  [caminho completo]/[nome-base-cap9].pdf  (com cabeçalho e numeração de páginas)
    MD    ->  [caminho completo]/[nome-base-cap9].md

  Capítulo 10:
    DOCX  ->  [caminho completo]/[nome-base-cap10].docx
    PDF   ->  [caminho completo]/[nome-base-cap10].pdf  (com cabeçalho e numeração de páginas)
    MD    ->  [caminho completo]/[nome-base-cap10].md

Para importar no Google Drive:
  1. Arraste os arquivos .docx para o Google Drive
